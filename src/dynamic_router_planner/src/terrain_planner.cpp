...
void TerrainPlanner::Init(const ros::NodeHandle& nh, const TerrainPlannerParams& params) {
    ...
    col_num_ = row_num_;
    TerrianNodePtr init_terrian_node_ptr = NULL;
    ...
    terrain_grids_ = std::make_unique<grid_ns::Grid<TerrianNodePtr>>(grid_size, init_terrian_node_ptr, grid_origin, grid_resolution, 3);
    this->AllocateGridNodes(); 
    viz_path_stack_.clear();

    local_path_pub_   = nh_.advertise<Marker>("/local_terrain_path_debug", 5);
    terrian_map_pub_  = nh_.advertise<sensor_msgs::PointCloud2>("/local_terrain_map_debug", 5);
}
...
void TerrainPlanner::GridVisualCloud() {
    ...
    terrian_map_pub_.publish(msg_pc);
}
...
bool TerrainPlanner::PlanPathFromPToP(const Point3D& from_p, const Point3D& to_p, PointStack& path) {
    ...
    const TerrianNodePtr start_node_ptr = terrain_grids_->GetCell(terrain_grids_->Sub2Ind(start_sub));
    const TerrianNodePtr end_node_ptr = terrain_grids_->GetCell(terrain_grids_->Sub2Ind(end_sub));
    const Point3D unit_axial = (to_p - from_p).normalize();
    const float ndist = (to_p - from_p).norm();

    // Lambda function
    auto InCylinder = [&](const TerrianNodePtr& tnode_ptr) {
        const Point3D vec = tnode_ptr->position - from_p;
        float proj_scalar = vec * unit_axial;
        if (proj_scalar < - DPUtil::kNavClearDist || proj_scalar > ndist + DPUtil::kNavClearDist) {
            return false;
        }
    ...
    std::array<int, 8> dx = { 1, 1, 0,-1,-1, 0, 1 };
    std::array<int, 8> dy = { 0, 1, 1, 0, 1,-1,-1 };
    start_node_ptr->gscore = 0.0;
    std::unordered_set<int> open_set;
    std::priority_queue<TerrianNodePtr, std::vector<TerrianNodePtr>, TNodeptr_fcomp> open_queue;
    ...
    while (true) {
        ...
        const TerrianNodePtr current = open_queue.top();
        ...
        for (int i=0; i<8; i++) {
            ...
            const TerrianNodePtr neighbor = terrain_grids_->GetCell(terrain_grids_->Sub2Ind(csub));
...
void TerrainPlanner::ExtractPath(const TerrianNodePtr& end_ptr, PointStack& path) {
    path.clear();
    TerrianNodePtr cur_ptr = end_ptr;
    while (true) {
...
void TerrainPlanner::VisualPaths() {
    ...
    auto DrawPath = [&](const PointStack& path) {
        ...
        geometry_msgs::Point last_p = DPUtil::Point3DToGeoMsgPoint(path[0]);
        for (int i=1; i<path.size(); i++) {
            terrain_paths_marker.points.push_back(last_p);
            last_p = DPUtil::Point3DToGeoMsgPoint(path[i]);
            terrain_paths_marker.points.push_back(last_p);
        }
    };
