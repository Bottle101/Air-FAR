...
public:
    ...
    static bool  IsNavpoint;
    static bool  IsSimulation;
    static bool  IsTrajectory;
    ...
    static PointCloudPtr new_obs_cloud_;            // new obstacle Point Cloud
    ...
    // kdTree cloud
    // static PointKdTreePtr kdtree_freespace_cloud_;
    // static PointKdTreePtr kdtree_obsspace_cloud_;
    static PointKdTreePtr kdtree_new_cloud_;
    static PointKdTreePtr kdtree_filter_cloud_;        
    // PCL Util clouds
    static PointCloudPtr free_flat_cloud_;
    static std::vector<float> layerIdx2Height_;
    ...
    */
    ...
    static void ExtractSurroundCloud(const PointCloudPtr& worldObsCloudIn, const PointCloudPtr& worldFreeCloudIn,
    ...
    static void ExtractFreeAndObsFromScanCloud(const PointCloudPtr& scanCloudIn,
    ...
    static bool IsPointInToleratedHeight(const Point& p,
                                         const float& height=DPUtil::kTolerZ) 
    {
        if (abs(p.z - DPUtil::robot_pos.z) < height) return true;
        return false;
    };
    ...
    static float NoiseCosValue(const float& dot_value, const bool& is_large, const float& noise = DPUtil::kAngleNoise);
    ...
    static bool IsOutReachNode(const NavNodePtr& node_ptr) {
        for (const auto& cnode : node_ptr->connect_nodes) {
            if (!cnode->is_near_nodes) return true;
        }
        return false;
    }
    ...
    static Point NormalizePXY(const Point& p) {
        ...
        if (norm > DPUtil::kEpsilon) {
            return_p.x /= norm, return_p.y /= norm;
        } else {
            ROS_ERROR("DP: Point XY normalization fails, vector norm is too small.");
        }
        return return_p;
    }
    ...
    static bool PointInsideAPoly(const std::vector<Point>& poly, const Point& p) {
        ...
        int npol = poly.size();
        if (npol < 3) {
            ROS_ERROR("The vertices number of a polygon is less than 3.");
            return false;
        }
    ...
    template <typename Point>
    static bool IsConvexPointFromFree(const std::vector<Point>& poly, const Point& ev_p, const Point& free_p) {
        const bool in_or_out = PointInsideAPoly(poly, free_p);
        if (!DPUtil::PointInsideAPoly(poly, ev_p) == in_or_out) {
            return true;
        } 
        return false;
    }
    ...
    static bool IsPointInVetexAngleRestriction(const NodeType& node_type, const Point3D end_p) {
        if (node_type->free_direct == NodeFreeDirect::PILLAR) return true;
        const cv::Point2f p(end_p.x, end_p.y);
        const cv::Point2f center_p(node_type->position.x, node_type->position.y);
        const cv::Point2f surf_dir1(node_type->surf_dirs.first.x, node_type->surf_dirs.first.y);
        const cv::Point2f surf_dir2(node_type->surf_dirs.second.x, node_type->surf_dirs.second.y);
        if (std::hypotf(p.x - center_p.x, p.y - center_p.y) < 1e-5) {
            ROS_WARN("The two vertex are too close, connection fails.");
            return false;
        }
        const cv::Point2f p_dir = DPUtil::NormalizePXY(p - center_p);
        cv::Point2f temp_opposite_dir = -surf_dir2;
        float thred_dot_value = surf_dir1.dot(temp_opposite_dir) - 0.1;
        if (p_dir.dot(surf_dir1) > thred_dot_value && p_dir.dot(temp_opposite_dir) > thred_dot_value) {
            return true;
        }
        temp_opposite_dir = -surf_dir1;
        thred_dot_value = surf_dir2.dot(temp_opposite_dir) - 0.1;
