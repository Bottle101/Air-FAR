...
void ScanHandler::Init(const ScanHandlerParams& params) {
    scan_params_ = params;
    row_num_ = std::ceil(scan_params_.sensor_range / scan_params_.voxel_size);
...
void ScanHandler::UpdateRobotPosition(const Point3D& robot_pos) {
    ...
    grid_origin.x() = robot_pos.x - (scan_params_.voxel_size * row_num_) / 2.0;
    grid_origin.y() = robot_pos.y - (scan_params_.voxel_size * col_num_) / 2.0;
    grid_origin.z() = robot_pos.z - (scan_params_.voxel_size * level_num_) / 2.0;
...
void ScanHandler::SetCurrentScanCloud(const PointCloudPtr& scanCloudIn) {
    ...
    const int N_IF = scan_params_.inflate_size;
...
void ScanHandler::SetSurroundObsCloud(const PointCloudPtr& obsCloudIn, const bool& is_filter_cloud) {
    ...
    if (is_filter_cloud) DPUtil::FilterCloud(obsCloudIn, scan_params_.voxel_size);
...
void ScanHandler::SetRayCloud(const Eigen::Vector3i& point_sub) {
    ...
    sign_sub.x() = DPUtil::Signum(dir_sub.x()), sign_sub.y() = DPUtil::Signum(dir_sub.y()), sign_sub.z() = DPUtil::Signum(dir_sub.z());
